...
library MinimalProxyStore {
     ...
     * @param context the data to be stored in the proxy
    ...
    /**
     * @dev Deploys and returns the address of a clone stored context data that mimics the behaviour of `implementation`
     * This function uses the create2 opcode and a `salt` to deterministically deploy the clone
     * Using the same `implementation` and `salt` multiple time will revert, since the clone cannot by deployed twice at the same address
     *
     * @param implementation the implementation to delegate to
     * @param context the data to be stored in the proxy
     * @return instance the address of the deployed proxy
