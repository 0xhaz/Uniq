...
library BinarySearchTree {
    uint256 private constant TIME_EXTENSION = 50400; // 14 hours
    ...
    struct Tree {
        ...
        mapping(uint256 => uint256[]) futureExpires; // map from last divisible root to a list of expires since last divisible root
    }

    // helper function for insert
    function insertHelper(Tree storage self, uint256 newValue, uint256 nodeId) public {
        // current node
    ...
    function insert(Tree storage self, uint256 newValue) public {
        ...
        if (self.root == 0) {
            ...
            self.futureExpires[self.root].push(newValue);
        } else {
            insertHelper(self, newValue, self.root);
        }
    }

    function returnListHelperEx(Tree storage self, uint256 start, uint256 end, uint256 nodeId, uint256 extension)
        public
    {
        if (start <= end && end < extension) {
            ...
            if (curNode.value != 0) {
                ...
                if (curNode.value <= end && curNode.value >= start) {
                    if (
                        self.rootToList[self.root].length == 0
                            || (
                                self.rootToList[self.root].length > 0
                                    && self.rootToList[self.root][self.rootToList[self.root].length - 1] != curNode.value
                            )
                    ) {
                ...
                if (curNode.value <= extension && curNode.value > end) {
                    if (
                        self.futureExpires[self.root].length == 0
                            || (
                                self.futureExpires[self.root].length > 0
                                    && self.futureExpires[self.root][self.futureExpires[self.root].length - 1] != curNode.value
                            )
                    ) {
                        self.futureExpires[self.root].push(curNode.value);
                    }
                }
    ...
    function deleteNodeHelper(Tree storage self, uint256 deleteValue, uint256 nodeId)
        public
    ...
    function deleteLeaf(Tree storage self, uint256 nodeId) public returns (uint256 newNodeId) {
        Node memory curNode = self.nodes[nodeId];
        if (curNode.left != 0) {
    ...
    function deleteNode(Tree storage self, uint256 deleteValue) public returns (uint256 newRoot) {
    ...
    function trimTreeHelper(Tree storage self, uint256 start, uint256 end, uint256 nodeId) public {
        if (start <= end) {
            Node memory curNode = self.nodes[nodeId];
            if (curNode.value != 0) {
                if (curNode.value < start) {
    ...
    function trimTree(Tree storage self, uint256 start, uint256 end) public returns (uint256 newRoot) {
        if (start <= end) {
    ...
    function processExpiresListTrimTree(Tree storage self, uint256 start, uint256 end) public {
        if (self.root != 0) {
            delete self.futureExpires[self.root];
            self.futureExpires[self.root].push(end);
    ...
    function getExpiresList(Tree storage self) public view returns (uint256[] memory) {
        return self.rootToList[self.rootLast];
    }

    function getFutureExpiresList(Tree storage self) public view returns (uint256[] memory) {
        return self.futureExpires[self.rootLast];
    }
}
